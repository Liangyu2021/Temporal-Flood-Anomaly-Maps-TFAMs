...
#  ★ 配置区 ★
# ══════════════════════════════════════════════════════════════
base_tcev_dir   = "/home/data/ql2024/flood-tly/greek/data/tcev_seasonal_filtered/autumn"
base_sar_dir    = "/home/data/ql2024/flood-tly/greek/data/SARdata_filtered"
base_output_dir = "/home/data/ql2024/flood-tly/greek/data/event_Fx"
...
DATE_START = '20240901'   # 起始日期（含）
DATE_END   = '20241201'   # 结束日期（含）
# ─────────────────────────────────────────────────────────────
...
REFERENCE_DATE = '20141012'   # 例：'20141008'，或 None
# ─────────────────────────────────────────────────────────────
...
def detect_tcev_file_prefix(tcev_dir):
    """
    扫描 tcev 子目录，自动探测文件名中的前缀部分。
    例如 L1_alpha_flood_01.tif     → 前缀 flood_01
    例如 L1_alpha_poyanghu_03.tif  → 前缀 poyanghu_03

    返回:
        str: 探测到的文件前缀（如 "flood_01"）
    异常:
        FileNotFoundError: 目录不存在或未找到匹配文件
    """
    tcev_dir = str(tcev_dir)

    if not os.path.exists(tcev_dir):
        raise FileNotFoundError(f"TCEV目录不存在: {tcev_dir}")

    candidates = []
    for fname in os.listdir(tcev_dir):
        m = re.match(r'^L1_alpha_(.+)\.tif$', fname, re.IGNORECASE)
        if m:
            candidates.append(m.group(1))

    if not candidates:
        actual = [f for f in os.listdir(tcev_dir) if f.endswith('.tif')]
        raise FileNotFoundError(
            f"在 {tcev_dir} 中未找到 'L1_alpha_*.tif' 格式的文件。\n"
            f"实际TIF文件: {actual}\n"
            "请确认文件命名格式。"
        )
...
def process_event_image(event_db_path, alpha1_path, beta1_path, alpha2_path, beta2_path,
                        output_dir, reference_profile=None,   # ★ 新增：外部传入基准profile
...
def process_event_folder(event_folder, alpha1_path, beta1_path, alpha2_path, beta2_path,
    ...
    for i, event_file in enumerate(event_files, 1):
        ...
        output_path = process_event_image(
            ...
            reference_profile=reference_profile,   # ★ 透传
...
def batch_process_folders(base_tcev_dir, base_sar_dir, base_output_dir,
                          start_num, end_num,
                          date_start=None, date_end=None,
                          reference_date=None,              # ★ 新增：透传基准日期
                          n_processes=None, chunk_size=100,
                          reverse_db=True, skip_existing=True):
    """
    批量处理 start_num ~ end_num 编号的数据。

    两类前缀均自动探测，无需手动填写：
      · SAR 子目录前缀  ← 扫描 base_sar_dir，匹配 '*_NN' 目录名
      · TCEV 文件前缀   ← 扫描每个 tcev_NN 子目录，匹配 'L1_alpha_*.tif'

    reference_date:
      在每个 SAR 子文件夹中自动找到该日期的影像作为空间基准（范围+分辨率）。
      所有其他影像及 TCEV 参数均重采样/重投影到该基准。
      None = 各影像自身空间参考（原有行为）。
    """
    ...
    for i in range(start_num, end_num + 1):
        ...
        try:
            ...
            # ---- 自动探测 TCEV 文件前缀（每个子目录独立探测）----
            #file_prefix = detect_tcev_file_prefix(tcev_folder)
            #print(f"[自动探测] TCEV文件前缀: {file_prefix}")

            alpha1_path = tcev_folder / f"L1_alpha.tif"
            beta1_path  = tcev_folder / f"L1_beta.tif"
            alpha2_path = tcev_folder / f"L2_alpha.tif"
            beta2_path  = tcev_folder / f"L2_beta.tif"
            ...
            process_event_folder(
                ...
                date_end=date_end,
                reference_date=reference_date,       # ★ 透传
...
def main():
    batch_process_folders(
        ...
        date_start      = DATE_START,
        date_end        = DATE_END,
        reference_date  = REFERENCE_DATE,   # ★ 传入基准日期
        n_processes     = N_PROCESSES,
        chunk_size      = CHUNK_SIZE,
        reverse_db      = REVERSE_DB,
        skip_existing   = SKIP_EXISTING
